...
test("uses English as the default commit language", () => {
	assert.deepEqual(normalizeConfig(undefined), { commitLanguage: "english" });
	assert.deepEqual(normalizeConfig({ commitLanguage: "invalid" }), {
...
test("reads and atomically writes the selected language", async () => {
	...
	try {
		await writeConfig({ commitLanguage: "chinese" }, configPath);
		assert.deepEqual(await readConfig(configPath), { commitLanguage: "chinese" });
